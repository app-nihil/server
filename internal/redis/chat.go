...
func (c *Client) JoinChat(ctx context.Context, token, joinerUUID string) (*Chat, error) {
	...
	if invitation.CreatorUUID == joinerUUID {
		return nil, fmt.Errorf("cannot join your own chat")
	}

	chat, err := c.GetChat(ctx, invitation.ChatUUID)
	if err != nil {
		return nil, err
	}
	...
	chatKey := fmt.Sprintf("chat:%s", invitation.ChatUUID)
	if err := c.rdb.Set(ctx, chatKey, chatJSON, 0).Err(); err != nil {
		return nil, fmt.Errorf("failed to update chat: %w", err)
	}

	invitation.Used = true
	invJSON, _ := json.Marshal(invitation)
	invKey := fmt.Sprintf("invite:%s", token)
	c.rdb.Set(ctx, invKey, invJSON, time.Hour)

	userChatsKey := fmt.Sprintf("user_chats:%s", joinerUUID)
	if err := c.rdb.SAdd(ctx, userChatsKey, invitation.ChatUUID).Err(); err != nil {
		return nil, fmt.Errorf("failed to add chat to user list: %w", err)
	}
