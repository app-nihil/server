...
func (c *Client) ClaimActivationCode(ctx context.Context, code, deviceUUID, publicKey string) (*Subscription, error) {
...
codeKey := fmt.Sprintf("code:%s", code)
c.rdb.Set(ctx, codeKey, codeJSON, time.Hour)
...
if codeType == "duo_owner" || codeType == "duo_guest" {
return "duo"
}
return "solo"
}
